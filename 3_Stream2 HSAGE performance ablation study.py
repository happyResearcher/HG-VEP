...
#%% Build HSAGE model #  https://pytorch-geometric.readthedocs.io/en/latest/notes/heterogeneous.html
...
for Feature in Features:
    ...
    model = HeteroGNN(data.metadata(), hidden_channels=128, out_channels=2,num_layers=2)
    modelname=["HeteroGNN"]
    ...
    model_extract = HeteroGNN(data.metadata(), hidden_channels=128, out_channels=2,num_layers=2)
    model_extract.load_state_dict(torch.load(best_model_path))

    #%% save result
    classifier_name = modelname[0]

    parameters = "hidden_channels=128,num_layer=2, lr=0.005, weight_decay=0.0005,patience=10, min_delta=0.000001"
