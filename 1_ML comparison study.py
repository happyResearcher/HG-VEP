...
X_train = data['Vulnerability'].x[data['Vulnerability'].train_mask].numpy()
# Putting response variable to y
y_train = data['Vulnerability'].y[data['Vulnerability'].train_mask].numpy()

# Putting feature variable to X
X_test = data['Vulnerability'].x[data['Vulnerability'].test_mask].numpy()   #
# Putting response variable to y
...
# plot all 8 figures:
result_dir=cur_dir+'/data/ML_classification_performance.csv'
data = np.load(plot_dir, allow_pickle=True).item()

for k in range(len(classifier_name_list)):
    print(classifier_name_list[k])

    [fpr, tpr] = data[classifier_name_list[k]]
    roc_auc = auc(fpr, tpr)
